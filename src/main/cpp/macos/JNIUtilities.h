...
#define GET_CLASS(dst_var, cls) \
     if (dst_var == NULL) { \
         dst_var = env->FindClass(cls); \
         if (dst_var != NULL) dst_var = env->NewGlobalRef(dst_var); \
     } \
...
#define GET_METHOD(dst_var, cls, name, signature) \
     if (dst_var == NULL) { \
         dst_var = env->GetMethodID(cls, name, signature); \
     } \
...
#define GET_STATIC_METHOD(dst_var, cls, name, signature) \
     if (dst_var == NULL) { \
         dst_var = env->GetStaticMethodID(cls, name, signature); \
     } \
     LOG_NULL(dst_var, name); \
     CHECK_NULL(dst_var);
...
#define GET_FIELD(dst_var, cls, name, signature) \
     if (dst_var == NULL) { \
         dst_var = env->GetFieldID(cls, name, signature); \
     } \
     LOG_NULL(dst_var, name); \
     CHECK_NULL(dst_var);
...
#define GET_STATIC_FIELD_RETURN(dst_var, cls, name, signature, ret) \
     if (dst_var == NULL) { \
         dst_var = env->GetStaticFieldID(cls, name, signature); \
     } \
     LOG_NULL(dst_var, name); \
     CHECK_NULL_RETURN(dst_var, ret);
